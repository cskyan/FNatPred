...
folder_path0 = 'data/IvsIV'
folder_path = os.path.join(script_dir, folder_path0)
# Initialize the array of storage file names
file_names = []
train_datay = None
train_datax = None
roc_df = pd.DataFrame(columns=['Model', 'auroc', 'aupr', 'diseaseType'])
results_cv = []  # Store model results

# Iterate through the folder for all files
for i, filename in enumerate(sorted(os.listdir(folder_path))):
    ...
    if train_datay is not None and train_datax is not None:
        ...
        train_data_y_encoded = [0 if label == "StageIV" else 1 for label in train_data_y]
        ...
        #
        # for fold_num, (train_idx, valid_idx) in enumerate(kfold.split(train_data_X), 1):
        #     train_x, train_y = train_data_X.iloc[train_idx], train_data_y_encoded_series.iloc[train_idx]
        #     valid_x, valid_y = train_data_X.iloc[valid_idx], train_data_y_encoded_series.iloc[valid_idx]
        #
        #     grid_search.fit(train_x, train_y, eval_set=[(train_x, train_y), (valid_x, valid_y)],
        ...
        #         'aupr': pr_value_gbm,
        #         'rep': f'Fold{fold_num}',
        #         'diseaseType': file_names[i // 2],
        #         'sampleType': 'Stage I vs IV'
        #     }
        #     row_xgb = {
        #         'model': 'XGBoost',
        #         'auroc': auc_score_xgb,
...
#         model_names = ['fmodel']
#         roc_values = [auc_value_fmodel]
#         pr_values = [pr_value_fmodel]
#         file_names = [file_names[i - 1]]* len(model_names)

        # Add the model name, ROC ,PR, and diseaseType to the roc_df
        for model, roc, pr, diseaseType in zip(model_names, roc_values, pr_values, disease):
            roc_df = roc_df.append({'Model': model, 'auroc': roc, 'aupr': pr, 'diseaseType': diseaseType},
                                   ignore_index=True)
        print(roc_df)
...
output_file1 = os.path.join(output_folder, 'IvsIV_FmodelvsOthers_value.csv')
# output_file2 = os.path.join(output_folder, 'IvsIV_cv_Cancers_random.csv')
roc_df.to_csv(output_file1, index=False)
# results_cv_df.to_csv(output_file2, index=False)
